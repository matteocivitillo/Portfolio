...
type PixelatedCanvasProps = {
  ...
  fadeSpeed?: number;
  /** Enable halftone mode: dot size varies with brightness. */
  halftone?: boolean;
  /** Contrast boost factor for halftone (1 = normal, 2+ = high contrast). */
    ...
    const compute = () => {
      ...
      for (let y = 0; y < offscreen.height; y += cellSize) {
        ...
        for (let x = 0; x < offscreen.width; x += cellSize) {
          ...
          // Compute brightness and apply contrast boost for halftone
          let brightness = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
          if (halftone && contrastBoost > 1) {
            brightness = (brightness - 0.5) * contrastBoost + 0.5;
            brightness = Math.max(0, Math.min(1, brightness));
          }
          // For halftone: compute darkness (inverse of brightness)
          // Bright pixels → darkness near 0 → skip (clean background)
          // Dark pixels → darkness near 1 → large dots
          const darkness = 1 - brightness;

          samples.push({ x, y, r, g, b, a, drop, seed, brightness, darkness });
        }
      }
    ...
    img.onload = () => {
      ...
      if (!interactive) {
        ...
        for (const s of samples) {
          ...
          if (halftone) {
            // White-bg halftone: skip bright pixels (clean background)
            if (s.darkness < 0.22) continue;
            const maxRadius = cellSize / 2;
            const radius = s.darkness * maxRadius;
            ctx.globalAlpha = Math.min(1, s.darkness * 1.5);
      ...
      const animate = () => {
        ...
        for (const s of samples) {
          ...
          if (halftone) {
            // White-bg halftone: skip bright pixels
            if (s.darkness < 0.22) continue;
            const maxRadius = cellSize / 2;
            const radius = s.darkness * maxRadius;
            ctx.globalAlpha = Math.min(1, s.darkness * 1.5);
